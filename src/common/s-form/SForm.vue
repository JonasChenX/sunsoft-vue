        ...
        >
            ...
            <template v-if="formItem.type == 'inputText'">
                ...
                <v-text-field
                    ...
                    :error-messages="$v[formItem.modelName]?.$errors.map(e => e.$message)"
                ></v-text-field>
            </template>
            ...
            <template v-if="formItem.type == 'slider'">
                ...
                <v-slider
                    :label="formItem.label"
                    v-model="formData[formItem.modelName]"
                    :max="Object.keys(formItem.ticksItem).length - 1"
...
import {FormGroupsItem, FormValidation} from "@/common/s-form/s-form-type";
defineOptions({
    name: "s-form"
});
...
//表單顯示物件初始化
const formAttributes = ref<FormGroupsItem[]>([]);

onMounted(()=>{
    formAttributes.value = init(props.formConfig.groups)
})

const init = (groups: FormGroupsItem[]) => {
